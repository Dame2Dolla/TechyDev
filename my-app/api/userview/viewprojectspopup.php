...
$stmt = $conn->prepare("SELECT pjt.project_ID, pjt.projectName, pjt.projectDesc, pjt.date_end, pjt.is_ongoing FROM tbl_Users us JOIN tbl_Users_Projects uspjt ON us.user_ID = uspjt.user_ID_pk_fk JOIN tbl_Projects pjt ON pjt.project_ID = uspjt.project_ID_pk_fk WHERE us.user_ID = ?;");
...
if ($result->num_rows === 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        echo  '<img class="edit-button-half" src="./images/edit.svg" width="100%" height="100%" onclick="openEditProjectPopup(' . $row["project_ID"] . ', \'' . addslashes($row["projectName"]) . '\', \'' . addslashes($row["projectDesc"]) . '\', \'' . $row["date_end"] . '\', ' . ($row["is_ongoing"] == 1 ? 'true' : 'false') . ')" />';
        echo   '<div>';
        echo '<h5 class="popup-education-university-name pl-1">' . $row["projectName"] . '</h5>';
        echo '<p class="popup-education-university-text pl-1">' . $endDate . '</p>';
        echo '</div>';
        echo '</div>';
    }
}
